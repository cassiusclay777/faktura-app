...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const rawFixNamesProvider =
      (formData.get("fixNamesProvider") as string) || "gemini";
    /** Dříve „deepseek“ – nyní OpenRouter (OpenAI-kompatibilní). */
    const fixNamesProvider =
      rawFixNamesProvider === "deepseek" ? "openrouter" : rawFixNamesProvider;
    ...
    if (fixNames) {
      let corrected;
      if (fixNamesProvider === "openrouter") {
