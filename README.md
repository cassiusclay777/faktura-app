...
- **Korekce názvů** – v záložce Faktura zvol **Gemini** (`GEMINI_API_KEY`, volitelně vyhledávání na webu) nebo **OpenRouter** (`OPENROUTER_API_KEY`, výchozí model `openai/gpt-4o-mini`, volitelně `OPENROUTER_MODEL`, `OPENROUTER_API_BASE`). Volitelně `OPENROUTER_HTTP_REFERER` a `OPENROUTER_APP_TITLE` (doporučuje OpenRouter). Vlastní instrukce v poli pod tím platí pro oba.
